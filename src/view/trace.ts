...
export class Trace {
  ...
  // Dispose GPU resources associated with this trace
  dispose() {
    try {
      if (this.line && this.line.geometry && (this.line.geometry as any).dispose) {
        ;(this.line.geometry as any).dispose()
      }
    } catch {}
    try {
      const mat: any = this.line && (this.line as any).material
    ...
    try {
      if (this.geometry && (this.geometry as any).dispose) {
