...
export class Grid {
  ...
  public generateLineSegments() {
    const isCarom = !TableGeometry.hasPockets;
    const points = [
      this.point(0, (-11.13 * R) / 0.5),
      this.point(0, (11.13 * R) / 0.5),
    ]

    const stepx = isCarom ? (TableGeometry.tableX / 4) : (TableGeometry.X / 4)
    const xs = [1, 2, 3, -1, -2, -3]
    const yedge = isCarom ? TableGeometry.tableY
                      : TableGeometry.tableY + R;
    xs.forEach((x) => {
      points.push(this.point(x * stepx, -yedge))
      points.push(this.point(x * stepx, yedge))
    })

    const stepy = isCarom ? (TableGeometry.Y / 2)
                      : ((TableGeometry.Y + R) / 2);
    const ys = [-1, 0, 1]
    const xedge = isCarom ? TableGeometry.tableX
                      : TableGeometry.tableX + R;
    ys.forEach((y) => {
